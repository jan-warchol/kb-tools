...
| **A card for understanding a whole note**, the kind removed in 0.16 | A review that means reasoning through a whole note costs far more than a recall card and did not return it, so the cards went unreviewed; the quiz that would have graded them had already gone in 0.15 | The rest of the pipeline has settled, and there is a design that makes the review worth its cost |
| **Machine-graded free recall** | Adds friction where the habit is weakest, and recall review on a phone is most of what keeps it going. A first attempt, `/kb-quiz` over a note, was removed in 0.15 as not working well, and the understanding card it graded went with the kind in 0.16 | Self-graded review has been unbroken for a month, and a design addresses why the quiz fell short |
