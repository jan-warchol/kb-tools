...
| **Relating an item to what is already known** (the overlap check, the `related:` graph) | Needs a corpus large enough for a new note to plausibly duplicate an old one, and needs the index below to be affordable | A capture turns out to restate an existing note and nothing catches it |
